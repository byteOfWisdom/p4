...
def main():
        ...
        pressure_pa = lambda t: np.log(np.exp(pressure_log(t)) * 133)
        start_temp = 160 + 173.15
        end_temp = 180 + 173.15
        test_temps = np.linspace(start_temp,end_temp,500)
        std.default.plt_pretty ("Temperatur T / K", "Druck p / Pa")
        plt.plot(temperatures_k, np.exp(pressure_pa(temperatures_k)),label="Hg-Dampfdruckkurve",linestyle="--")
        plt.plot(test_temps,np.exp(pressure_pa(test_temps)),label = "Hg-Dampfdruckkurve im gemessenen Intervall")
        plt.legend(loc="best")
        if len(argv) >= 2:
            plt.savefig(argv[1])
        else:
